...
const Sidebar = ({isMenuOpen,setIsMenuOpen}) => {
  ...
  const deleteChat = async (e,chatId) =>{
    try {
      e.stopPropagation();
      const confirm = window.confirm("Are you sure to delete chat?");
      if(!confirm) return;
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <div className='flex items-center gap-2 p-3 mt-4  border border-gray-400 dark:border-white/20 rounded-md'>
        ...
        <input onChange={(e) => setSearch(e.target.value)} value={search} type="text" placeholder='Seach Conversation' className='text-xs placeholder:text-gray-400 outline-none' />
      </div>
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
          chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) :
            ...
            <div onClick={()=>{navigate("/"); setSelectedChat(chat); setIsMenuOpen(false)}}
             key={chat._id} className='p-2 px-4 dark:bg-[#57317c]/10 border border-gray-300 dark:border-[#80609f]/15 rounded-md cursor-pointer flex justify-between group hover:border-gray-600'>
              <div>
              ...
              <img onClick={e=>toast.promise(deleteChat(e,chat._id),{loading:"deleting ....."})} src={assets.bin_icon} alt="delete_icon" className='hidden group-hover:block w-4 cursor-pointer not-dark:invert' />
            </div>
          ))
        }
      </div>
      ...
      <div onClick={() => {navigate('/credits');setIsMenuOpen(false)}} className=' flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
          ...
          {!user?.credits && <p className='text-xs text-gray-400 text-red-500'>Purchase credits to use NeuroChat</p>}
        </div>
      </div>
      ...
      <div onClick={() => navigate('/community')} className=' flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer group'>
        <img src={assets.user_icon} className='w-7 rounded-full' alt="" />
          <p className='flex-1 text-sm dark:text-primary truncate'>{user?user.name:"Please Login"}</p>
